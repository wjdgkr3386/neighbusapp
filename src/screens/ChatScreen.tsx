...
const ChatScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const renderFriend = ({ item }: { item: Friend }) => (
    <TouchableOpacity style={styles.friendItem} onPress={() => handleFriendClick(item)} activeOpacity={0.7}>
      <View style={styles.friendAvatar}><Text style={styles.friendAvatarText}>{item.name[0]}</Text></View>
      <View style={styles.friendInfo}>
        <Text style={styles.friendName}>{item.name}</Text>
        <View style={styles.friendLocation}><Text style={styles.locationIcon}>📍</Text><Text style={styles.friendLocationText}>{item.location}</Text></View>
  ...
  const renderChatRoom = ({ item }: { item: ChatRoom }) => (
    <TouchableOpacity style={styles.chatRoomItem} onPress={() => setSelectedChat(item.id)} activeOpacity={0.7}>
      <View style={styles.chatRoomAvatar}><Text style={styles.chatRoomAvatarText}>{item.name[0]}</Text></View>
  ...
  const renderMessage = ({ item }: { item: ChatMessage }) => (
    <View style={[styles.messageContainer, item.isMe ? styles.myMessageContainer : styles.otherMessageContainer]}>
      ...
      <View style={[styles.messageBubble, item.isMe ? styles.myMessageBubble : styles.otherMessageBubble]}>
        <Text style={[styles.messageText, item.isMe ? styles.myMessageText : styles.otherMessageText]}>{item.message}</Text>
      </View>
  ...
  const renderChatDetail = () => {
    ...
    return (
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.flexOne}>
        ...
        <FlatList data={DUMMY_MESSAGES} renderItem={renderMessage} keyExtractor={(item) => item.id} contentContainerStyle={styles.messagesList} inverted={false} />
        <View style={styles.inputContainer}>
          <TextInput style={styles.messageInput} placeholder="메시지를 입력하세요..." placeholderTextColor="#B8B8B8" value={messageInput} onChangeText={setMessageInput} multiline />
          <TouchableOpacity style={styles.sendButton} onPress={handleSendMessage} activeOpacity={0.7}><Text style={styles.sendButtonText}>전송</Text></TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
    );
  };
    ...
    <View style={styles.flexOne} {...panResponder.panHandlers}>
      ...
      <View style={styles.tabContainer}>
        <TouchableOpacity style={[styles.tab, activeTab === 'friends' && styles.activeTab]} onPress={() => setActiveTab('friends')} activeOpacity={0.7}>
          <Text style={[styles.tabText, activeTab === 'friends' && styles.activeTabText]}>친구 목록</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.tab, activeTab === 'chats' && styles.activeTab]} onPress={() => setActiveTab('chats')} activeOpacity={0.7}>
          <Text style={[styles.tabText, activeTab === 'chats' && styles.activeTabText]}>채팅 리스트</Text>
        </TouchableOpacity>
      </View>
      {activeTab === 'friends' ? (
        <>
          ...
          <FlatList data={FRIENDS} renderItem={renderFriend} keyExtractor={(item) => item.id} contentContainerStyle={styles.friendList} />
        </>
      ) : (
        <FlatList data={CHAT_ROOMS} renderItem={renderChatRoom} keyExtractor={(item) => item.id} contentContainerStyle={styles.chatRoomList} />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  flexOne: { flex: 1 },
  wrapper: { flex: 1 }, // This style is problematic, will be removed
  container: { flex: 1 }, // This style is problematic, will be removed
  header: { paddingHorizontal: 20, paddingVertical: 16, backgroundColor: '#FFFFFF', borderBottomWidth: 1, borderBottomColor: '#E5E5E5' },
  headerTitle: { fontSize: 18, fontWeight: '700', color: '#5C4A3A' },
  tabContainer: { flexDirection: 'row', backgroundColor: '#FFFFFF', borderBottomWidth: 1, borderBottomColor: '#E5E5E5' },
  tab: { flex: 1, paddingVertical: 14, alignItems: 'center', borderBottomWidth: 2, borderBottomColor: 'transparent' },
  activeTab: { borderBottomColor: '#9B7E5C' },
  tabText: { fontSize: 14, fontWeight: '500', color: '#8B7355' },
  activeTabText: { fontWeight: '700', color: '#5C4A3A' },
  friendButtonContainer: { flexDirection: 'row', padding: 12, backgroundColor: '#F5EDE4', gap: 12 },
  friendActionButton: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFFFFF', borderWidth: 1.5, borderColor: '#9B7E5C', borderRadius: 10, paddingVertical: 12, paddingHorizontal: 16 },
  friendActionIcon: { fontSize: 16, marginRight: 6 },
  friendActionText: { fontSize: 14, fontWeight: '600', color: '#9B7E5C' },
  friendList: { backgroundColor: '#FFFFFF', paddingBottom: 100 },
  friendItem: { flexDirection: 'row', padding: 16, borderBottomWidth: 1, borderBottomColor: '#F0F0F0', backgroundColor: '#FFFFFF' },
  friendAvatar: { width: 56, height: 56, borderRadius: 28, backgroundColor: '#9B7E5C', justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  friendAvatarText: { fontSize: 20, fontWeight: '600', color: '#FFFFFF' },
  friendInfo: { flex: 1, justifyContent: 'center' },
  friendName: { fontSize: 16, fontWeight: '600', color: '#5C4A3A', marginBottom: 4 },
  friendLocation: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 },
  locationIcon: { fontSize: 12, marginRight: 4 },
  friendLocationText: { fontSize: 12, color: '#8B7355' },
  friendStatus: { fontSize: 13, color: '#B8B8B8', fontStyle: 'italic' },
  chatRoomList: { backgroundColor: '#FFFFFF', paddingBottom: 100 },
  chatRoomItem: { flexDirection: 'row', padding: 16, borderBottomWidth: 1, borderBottomColor: '#F0F0F0', backgroundColor: '#FFFFFF' },
  chatRoomAvatar: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#9B7E5C', justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  chatRoomAvatarText: { fontSize: 18, fontWeight: '600', color: '#FFFFFF' },
  chatRoomInfo: { flex: 1 },
  chatRoomHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 },
  chatRoomName: { fontSize: 15, fontWeight: '600', color: '#5C4A3A' },
  chatRoomTime: { fontSize: 12, color: '#B8B8B8' },
  chatRoomFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  chatRoomLastMessage: { flex: 1, fontSize: 13, color: '#8B7355' },
  unreadBadge: { backgroundColor: '#9B7E5C', borderRadius: 10, minWidth: 20, height: 20, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 6, marginLeft: 8 },
  unreadText: { fontSize: 11, fontWeight: '600', color: '#FFFFFF' },
  chatHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 12, backgroundColor: '#FFFFFF', borderBottomWidth: 1, borderBottomColor: '#E5E5E5' },
  backButton: { padding: 4, width: 40 },
  backIcon: { fontSize: 24, color: '#5C4A3A' },
  chatHeaderTitle: { fontSize: 16, fontWeight: '600', color: '#5C4A3A', flex: 1, textAlign: 'center' },
  headerRight: { width: 40 },
  messagesList: { padding: 16, backgroundColor: '#F5EDE4' },
  ...
  senderName: { fontSize: 12, color: '#8B7355', marginBottom: 4, marginLeft: 4 },
  messageBubble: { padding: 12, borderRadius: 16, marginBottom: 4 },
  myMessageBubble: { backgroundColor: '#9B7E5C', borderBottomRightRadius: 4 },
  otherMessageBubble: { backgroundColor: '#FFFFFF', borderBottomLeftRadius: 4, borderWidth: 1, borderColor: '#E5E5E5' },
  messageText: { fontSize: 14, lineHeight: 20 },
  myMessageText: { color: '#FFFFFF' },
  otherMessageText: { color: '#5C4A3A' },
  messageTime: { fontSize: 11, color: '#B8B8B8', marginTop: 2 },
  inputContainer: { flexDirection: 'row', padding: 12, backgroundColor: '#FFFFFF', borderTopWidth: 1, borderTopColor: '#E5E5E5', alignItems: 'flex-end' },
  messageInput: { flex: 1, backgroundColor: '#FAFAFA', borderWidth: 1, borderColor: '#D8D0C8', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 10, fontSize: 14, color: '#333333', maxHeight: 100, marginRight: 8 },
  sendButton: { backgroundColor: '#9B7E5C', borderRadius: 20, paddingHorizontal: 20, paddingVertical: 10, justifyContent: 'center', alignItems: 'center' },
  sendButtonText: { color: '#FFFFFF', fontSize: 14, fontWeight: '600' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.5)', justifyContent: 'center', alignItems: 'center' },
  modalContainer: { width: '85%', backgroundColor: '#FFFFFF', borderRadius: 16, padding: 24, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 5 },
  ...
  requestsModalContainer: { width: '90%', maxHeight: '70%', backgroundColor: '#FFFFFF', borderRadius: 16, padding: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 5 },
  requestsHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
